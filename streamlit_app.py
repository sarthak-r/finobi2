...
def calculate_balance(super_bal, retirement_age, roi, annual_contribution):
    years = np.arange(retirement_age, 81)
    balance = np.zeros(len(years))
    balance[0] = super_bal
    for i in range(1, len(years)):
        balance[i] = balance[i - 1] * (1 + roi / 100) + annual_contribution
...
years, balance = calculate_balance(super_bal, retirement_age, roi, annual_contribution)
